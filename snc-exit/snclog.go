...
// exitRingBuf is tee'd into the logger; flushed every 5 min by startExitLogUploader.
var exitRingBuf *ringBuffer

func initLogging(path string) error {
	exitRingBuf = newRingBuffer(logRingSize)
	var sink io.Writer
	if path != "" {
		...
		sink = io.MultiWriter(os.Stderr, r, exitRingBuf)
	} else {
		sink = io.MultiWriter(os.Stderr, exitRingBuf)
	}
...
func startupBanner(listen, authURL string) {
	logInfof("snc-exit starting  listen=%s  auth=%s  pid=%d  time=%s",
		listen, authURL, os.Getpid(), time.Now().Format(time.RFC3339))
}
