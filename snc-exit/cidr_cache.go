...
func newCIDRCache(arbiterURL, nodeToken string) *cidrCache {
	return &cidrCache{
...
// lookupCountry returns the ISO-2 country code for ipStr, or "" if unknown.
func (c *cidrCache) lookupCountry(ipStr string) string {
	ip := net.ParseIP(ipStr)
	if ip == nil {
		return ""
	}
...
func (c *cidrCache) refresh() {
	req, err := http.NewRequest(http.MethodGet, c.arbiterURL+"/api/cidr/all", nil)
	if err != nil {
